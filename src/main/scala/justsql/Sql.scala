...
object Sql {
  ...
  def apply(f: SqlParamsBuilder => String): Sql = {
    val params = SqlParamsBuilder(ListBuffer.empty)
    val sql = f(params)
    new Sql(sql, params)
  }

  @inline def apply(sql: String): Sql =
    new Sql(sql, SqlParamsBuilder.empty)

}

case class Sql(sql: String, params: SqlParamsBuilder) {
